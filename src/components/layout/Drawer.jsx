  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 flex w-[82%] max-w-[340px] animate-drawer-in flex-col overflow-hidden border-r border-white/[.09] bg-surface-2 shadow-drawer">
        ...
        <div className="relative z-[2] flex items-center gap-3 px-4 pb-4 pt-12">
          <Avatar size="lg" glow />
          ...
          <Link
            ...
            className="grid h-8 w-8 flex-none place-items-center rounded-[10px] border border-line bg-black/[.28] text-ink-faint"
          >
        ...
        {students.length > 1 && (
          <div className="relative z-[2] px-4 pb-4">
            <span className="text-[10px] font-bold uppercase tracking-[.09em] text-ink-faint">
              Farzandlarim
            </span>
              ...
              {students.map((student) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex flex-1 items-center gap-2 rounded-[12px] border px-2.5 py-2 transition-colors",
                      active ? "border-carrot/[.34] bg-carrot/[.14]" : "border-line bg-black/[.26]",
                    )}
                  >
                    <Avatar src={student.photo_url} size="sm" />
                    <span className={cn("truncate text-[11.5px] font-bold", active ? "text-carrot-bright" : "text-ink-soft")}>
                ...
                const row = (
                  <>
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block truncate text-[10px] font-semibold text-ink-faint">
                        {note}
                      </span>
                    </span>
                    {soon ? (
                      <span className="flex-none rounded-full bg-black/30 px-2 py-0.5 text-[9px] font-bold text-ink-faint">
                        tez orada
                      </span>
                    ) : (
                      <ChevronRight size={15} strokeWidth={2.4} className="flex-none text-ink-faint" />
                    )}
                  </>
                );
        ...
        <div className="relative z-[2] border-t border-line px-4 pb-5 pt-3">
          <button
            type="button"
            onClick={logout}
            className="flex items-center gap-2.5 text-[13px] font-bold text-rose"
          >
            <LogOut size={17} strokeWidth={2.2} />
            Chiqish
          </button>
          <p className="mt-3 text-[9.5px] font-semibold text-ink-faint">Farzandim · 1.0.0</p>
        </div>
      </aside>
    </>
  );
