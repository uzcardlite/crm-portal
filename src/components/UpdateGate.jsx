...
// Where the latest version metadata lives (always fetched from the live web,
// even from inside the offline-first bundled app).
const VERSION_URL = "https://portal.ncrm.uz/app-version.json";
...
//    invites the parent to update.
//  • Old app (still loading portal.ncrm.uz remotely, from before the switch
//    to a local bundle): invites them to install the new official app.
...
export default function UpdateGate() {
  ...
  useEffect(() => {
    ...
    const remote = /portal\.ncrm\.uz/i.test(window.location.hostname);
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[9998] flex justify-center px-3 pt-[max(0.5rem,env(safe-area-inset-top))]">
      <div className="flex w-full max-w-md items-center gap-3 rounded-card border border-line bg-surface-2 px-4 py-3 text-ink shadow-glow-sm">
        <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-btn bg-carrot-grad text-ink">
          <Download size={18} />
        </span>
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-[11.5px] text-ink-soft">
            {info.notes || (isMigrate ? "Farzandim ilovasini yangilang" : `Versiya ${info.version}`)}
          </div>
        </div>
        <a
          href={info.url}
          target="_blank"
          rel="noreferrer"
          className="flex-shrink-0 rounded-btn bg-carrot-grad px-3.5 py-2 text-xs font-extrabold text-ink"
        >
          Yuklab olish
        </a>
        <button
          ...
          aria-label="Yopish"
          className="flex-shrink-0 text-ink-faint transition-colors hover:text-ink"
        >
          <X size={16} />
        </button>
      </div>
